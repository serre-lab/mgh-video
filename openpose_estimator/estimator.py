...
class TfPoseEstimator:
    ...
    def __init__(self, graph_path, target_size=(320, 240)):
        ...
        self.contents = tf.placeholder(tf.string)

        self.write_op = tf.io.write_file(self.filename, self.contents)
        ...
        self.persistent_sess.run(
            self.tensor_output,
            feed_dict={
                self.tensor_image: [np.ndarray(shape=(target_size[1], target_size[0], 3), dtype=np.float32)] * 128
            }
        )
    ...
    # (Kalpit): Changed annotations drawn based on COCO pose format
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                # added other condition to draw only required parts (Kalpit)
                if (i not in human.body_parts.keys()) or (i not in [0, 1, 2, 3, 4, 5, 6, 7, 14, 15, 16, 17]):
                    continue
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                # added for respecting the added condition above (Kalpit)
                if (pair[0] not in [0, 1, 2, 3, 4, 5, 6, 7, 14, 15, 16, 17]) or (pair[1] not in [0, 1, 2, 3, 4, 5, 6, 7, 14, 15, 16, 17]):
                    continue
    ...
    def inference(self, imgs, scales=None):
        npimg = imgs[0]
        if npimg is None:
        ...
        if len(imgs) < 128:
            ...
            diff = 128 - len(imgs)
            temp = imgs[-1].copy()
            imgs.extend([temp] * diff)
        assert len(imgs) == 128

        output = self.persistent_sess.run(self.tensor_output, feed_dict={self.tensor_image: imgs})
        if smaller:
        ...
        human_list = []
        for i in range(n):
            heatMats = output[i:i+1, :, :, :19]
            pafMats = output[i:i+1, :, :, 19:]
            logger.debug('inference-')
